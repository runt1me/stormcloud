...
def handle_register_new_device_request(request):
    logger.info("Server handling new device request.")

    customer_id      = db.get_customer_id_by_api_key(request['api_key'])
    if not customer_id:
        logger.warning("Could not find customer ID for the given API key: %s" % request['api_key'])
        response_code = 401
        response_data = json.dumps({
...
def handle_backup_file_request(request, file):
    logger.info("Server handling backup file request.")
    logger.info("File type: %s" % type(file))
    ...
    logger.info("Done writing file to %s" % path_on_server)
